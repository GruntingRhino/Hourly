...
test("supervisor verification is signed, school-domain authorized, and one-time", () => {
  ...
  assert.throws(() => consumeSupervisorVerificationToken(token, { secret: "secret", authorizedDomains: ["school.edu"], consumedIds }), /Expired verification token/);
...
test("supervisor verification rejects tampering and expiry", () => {
  ...
  assert.throws(() => consumeSupervisorVerificationToken(token, { secret: "secret", now: new Date("2026-08-11T00:00:00Z"), authorizedDomains: ["school.edu"], consumedIds: new Set() }), /Expired verification token/);
