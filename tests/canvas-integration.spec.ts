...
async function getCanvasStatus(ctx: APIRequestContext, token: string) {
  const res = await ctx.get(`${BASE}/api/integrations/canvas/status`, auth(token));
  expect(res.ok()).toBeTruthy();
  return res.json();
}

test.describe("Canvas integration foundation", () => {
  test.describe.configure({ mode: "serial" });

  test("school admin can connect Canvas and create preview/apply jobs", async () => {
    const ctx = await request.newContext();
    const adminToken = await getToken("schoolA");

    const connectRes = await ctx.post(`${BASE}/api/integrations/canvas/connect`, {
      ...auth(adminToken),
      data: {
        mode: "MOCK",
        displayName: "Canvas Mock Sandbox",
        baseUrl: "https://canvas.mock.local",
    ...
    expect(previewBody.summary.provider).toBe("CANVAS");
    expect(
    ...
    const applyRes = await ctx.post(`${BASE}/api/integrations/canvas/apply`, auth(adminToken));
    ...
    const status = await getCanvasStatus(ctx, adminToken);
  ...
  test("non-admin users are blocked from Canvas integration routes", async () => {
    const ctx = await request.newContext();
    const studentToken = await getToken("student1");
    const orgToken = await getToken("orgA");

    for (const token of [studentToken, orgToken]) {
      const res = await ctx.post(`${BASE}/api/integrations/canvas/connect`, {
        ...auth(token),
  ...
  test("school isolation is enforced for connection visibility and sync access", async () => {
    ...
    const statusA = await getCanvasStatus(ctx, schoolAToken);
    expect(statusA.connection).not.toBeNull();

    const statusBRes = await ctx.get(`${BASE}/api/integrations/canvas/status`, auth(schoolBToken));
    expect(statusBRes.ok()).toBeTruthy();
    const statusB = await statusBRes.json();
    expect(statusB.connection).toBeNull();

    const previewB = await ctx.post(`${BASE}/api/integrations/canvas/preview`, auth(schoolBToken));
  ...
  test("duplicate student email is logged and existing GoodHours user is linked", async () => {
    ...
    const errorsRes = await ctx.get(`${BASE}/api/integrations/canvas/errors`, auth(adminToken));
    ...
    const imported = cohorts.find((cohort: any) => cohort.name === "Canvas Biology 101 - Period 1");
    expect(imported).toBeTruthy();

    await ctx.dispose();
  });

  test("renamed Canvas course updates mapped cohorts", async () => {
    const ctx = await request.newContext();
    const adminToken = await getToken("schoolA");

    const reconnectRes = await ctx.post(`${BASE}/api/integrations/canvas/connect`, {
      ...auth(adminToken),
      data: {
        mode: "MOCK",
        displayName: "Canvas Mock Sandbox",
    ...
    expect(cohorts.some((cohort: any) => cohort.name === "Canvas Biology Honors - Period Red")).toBeTruthy();

    await ctx.dispose();
  });

  test("archived and deleted Canvas sections archive mapped cohorts", async () => {
    const ctx = await request.newContext();
    const adminToken = await getToken("schoolA");

    let connectRes = await ctx.post(`${BASE}/api/integrations/canvas/connect`, {
      ...auth(adminToken),
      data: { mode: "MOCK", mockScenario: "archived" },
    });
    expect(connectRes.status()).toBe(201);

    let applyRes = await ctx.post(`${BASE}/api/integrations/canvas/apply`, auth(adminToken));
    expect(applyRes.ok()).toBeTruthy();

    let cohortsRes = await ctx.get(`${BASE}/api/cohorts`, auth(adminToken));
    let cohorts = await cohortsRes.json();
    const archivedByScenario = cohorts.find((cohort: any) => cohort.name === "Canvas Service Lab - Advisory");
    expect(archivedByScenario?.status).toBe("ARCHIVED");

    connectRes = await ctx.post(`${BASE}/api/integrations/canvas/connect`, {
      ...auth(adminToken),
      data: { mode: "MOCK", mockScenario: "deleted" },
    });
    expect(connectRes.status()).toBe(201);

    applyRes = await ctx.post(`${BASE}/api/integrations/canvas/apply`, auth(adminToken));
    expect(applyRes.ok()).toBeTruthy();

    cohortsRes = await ctx.get(`${BASE}/api/cohorts`, auth(adminToken));
    cohorts = await cohortsRes.json();
    const deletedArchived = cohorts.find(
      (cohort: any) => cohort.status === "ARCHIVED" && /Canvas Biology .* - Period (Blue|2)/.test(cohort.name)
    );
  ...
  test("deleting a synced cohort clears stale mappings and allows Canvas to recreate it", async () => {
    const ctx = await request.newContext();
    const adminToken = await getToken("schoolA");
    ...
    expect(cohortsRes.ok()).toBeTruthy();
    cohorts = await cohortsRes.json();
    expect(cohorts.some((cohort: any) => cohort.name === "Canvas Biology 101 - Period 1")).toBeTruthy();
  ...
  test("sync errors and disconnect flow are visible to school admins", async () => {
    ...
    const disconnectRes = await ctx.post(`${BASE}/api/integrations/canvas/disconnect`, auth(adminToken));
  ...
  test("student removal upstream revokes pending invitations without cross-cohort damage", async () => {
    ...
    expect(cohortsRes.ok()).toBeTruthy();
    const cohorts = await cohortsRes.json();
    const biology = cohorts.find((cohort: any) => cohort.name === "Canvas Biology 101 - Period 1");
    expect(biology).toBeTruthy();

    const detailRes = await ctx.get(`${BASE}/api/cohorts/${biology.id}`, auth(adminToken));
    expect(detailRes.ok()).toBeTruthy();
    const detail = await detailRes.json();
    const removedInvite = detail.invitations.find((inv: any) => inv.email === "canvas.student.one@example.invalid");
